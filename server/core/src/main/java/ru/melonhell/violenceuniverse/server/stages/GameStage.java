...
public class GameStage implements Stage {
    ...
    public void onEnter(Session session) {
        Stage.super.onEnter(session);
        Game game = gameService.createGame(session.getUser());
        activeGames.put(session, game);
        game.addGameListener(new StageListener(game, session));
    ...
    private class StageListener implements GameEventListener {
        ...
        public void gameEnd(GameEntity game, List<GameRoundEntity> rounds, GameResult result) {
            ...
            scheduledExecutorService.schedule(() -> session.setStage(StageType.MENU), 5, TimeUnit.SECONDS);
        }
    }
}
